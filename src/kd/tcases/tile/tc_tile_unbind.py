...
from kd.tkcd.tile_msg import getDelPanelMsg, getBindTileMsg, getUnbdTileMsg
from tc_tile_base     import TcTileBase

logger = getLogger(__name__)

class TcTileUnbind(TcTileBase):

    def __init__(self, desc, tkcdUrl, tileSpecs):
        super(TcTileUnbind, self).__init__(desc, tkcdUrl, tileSpecs)
    ...
    def allTestCases(cls):
        ...
        tileSpecs = [ # RC,      msg
                      ...
                      [ RC.OK   ,getBindTileMsg(0xcacacaca, 1, 2, 1) ],
                      [ RC.OK   ,getBindTileMsg(0xcacacaca, 2, 1, 1) ],
                      [ RC.OK   ,getBindTileMsg(0xcacacaca, 2, 2, 1) ],
                      [ RC.OK   ,getUnbdTileMsg(0xcacacaca, 1, 2, 1) ],
                    ]
        tcases.append( cls( "Delete a tile", tkcdUrl, tileSpecs) )
        # del a panel
        tileSpecs = [ # RC,      msg
                      [ RC.OK   ,getBindTileMsg(0xcacacaca, 1, 1, 1) ],
                      [ RC.OK   ,getBindTileMsg(0xcacacaca, 1, 2, 1) ],
                      [ RC.OK   ,getBindTileMsg(0xcacacaca, 2, 1, 1) ],
                      [ RC.OK   ,getBindTileMsg(0xcacacaca, 2, 2, 1) ],
                      [ RC.OK   ,getDelPanelMsg(0xcacacaca, 1,    1) ],
                    ]
        tcases.append( cls( "Delete a panel", tkcdUrl, tileSpecs) )

        '''
        # bind 10 tile at same panel id
        tileSpecs = []
        for tileSetId in range(10, 20):
